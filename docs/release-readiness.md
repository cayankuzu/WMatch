...
| 32. Yeniden kullanım    |       8.0 | Ortak AppImage/transport/session purge korundu; bayat veri bandının iki kopyası `SwipeModal.banner` altında tekleşti; marka yıkama merdiveni tek kırmızıdan türetiliyor | Feature/UI source guard'ları; `check:contrast`; `check:visual-regression` | Görsel cihaz kanıtı yok | 30 ham `fontSize`/`borderRadius` hâlâ token'ı atlıyor (17'si dondurulmamış dosyada, `ui-typography-hierarchy-audit.md`) | Ölçülmedi | NO-GO    |
