...
BEGIN
  ...
  IF p_kind = 'like' THEN
    UPDATE public.swipe_quotas AS sq
    SET
      used_like_swipes = sq.used_like_swipes + 1,
      updated_at = v_now
    WHERE sq.user_id = p_user_id
      AND sq.used_like_swipes < GREATEST(0, p_like_limit)
    RETURNING sq.* INTO v_row;

    v_consumed := FOUND;
  ELSIF p_kind = 'dislike' THEN
    UPDATE public.swipe_quotas AS sq
    SET
      used_dislike_swipes = sq.used_dislike_swipes + 1,
      updated_at = v_now
    WHERE sq.user_id = p_user_id
      AND sq.used_dislike_swipes < GREATEST(0, p_dislike_limit)
    RETURNING sq.* INTO v_row;
  ...
  ELSIF p_kind = 'undo' THEN
    UPDATE public.swipe_quotas AS sq
    SET
      used_undos = sq.used_undos + 1,
      updated_at = v_now
    WHERE sq.user_id = p_user_id
      AND sq.used_undos < GREATEST(0, p_undo_limit)
    RETURNING sq.* INTO v_row;
...
BEGIN
  ...
  ELSIF p_kind = 'dislike' THEN
    UPDATE public.swipe_quotas AS sq
    SET
      used_dislike_swipes = GREATEST(0, sq.used_dislike_swipes - 1),
      updated_at = v_now
    WHERE sq.user_id = p_user_id
    RETURNING sq.* INTO v_row;
  ELSE
